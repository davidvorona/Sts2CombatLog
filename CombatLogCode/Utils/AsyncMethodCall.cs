...
using Sts2CombatLog.CombatLogCode.Extensions;
using HarmonyLib;
...
public static class AsyncMethodCall
{
    ...
    public static List<CodeInstruction> Create(ILGenerator generator, IEnumerable<CodeInstruction> code, MethodBase original, MethodInfo callMethod, MethodBase? beforeState = null, MethodBase? afterState = null, string? resultName = null)
    {
        ...
        Log.Info($"Patching StateMachineType: {stateMachineType.FullName}");
            ...
            else
            {
                switch (instruction.opcode.Value)
                {
                    ...
                    case (int)OpCodeValues.Brtrue: // What
                        Log.Warn("Unexpected Brtrue in jump section of state machine");
                        break;
                    case (int)OpCodeValues.Beq_S:
                    case (int)OpCodeValues.Beq: // Branch if current state == ?
                        if (checkState == null)
                        {
                            Log.Warn("Failed to evaluate beq, checkState null");
                            break;
                        }
        ...
        while (index < codeList.Count)
        {
            ...
            else if (endingState)
            {
                ...
                {
                    case -3:
                        Log.Warn("Found code between branching section and states");
        ...
        Log.Info($"Adding new state {newStateIndex} for method {callMethod.DeclaringType?.Name ?? "???"}.{callMethod.Name} {(before ? "before" : "after")} {targetMethod?.Name ?? targetState.Index.ToString()} with result type {resultType} ({resultName})");
    ...
    private static StateParamInfo MakeStateParameter(MethodBase method, Type stateMachineType, ParameterInfo param)
    {
        ...
        {
            Log.Info($"Using named result {param.Name} in method {method.Name}");
            addLoadInstructions = list =>
            {
    ...
    private class StateInfo //, MethodBase stateMethod, int indexLoadIndex)
    {
            ...
            ILGenerator generator)
        {
            ...
            if (callMethod.ReturnType.IsGenericType)
            {
                ...
                Log.Info($"Method to call has return type; making generic awaiter type [{returnType}]");

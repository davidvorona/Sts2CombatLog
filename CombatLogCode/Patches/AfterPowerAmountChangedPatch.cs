...
namespace Sts2CombatLog.CombatLogCode.Patches
{
    ...
    class AfterPowerAmountChangedPatch
    {
        ...
        private static async Task BeforeAfterPowerAmountChangedHook(PowerModel power, decimal amount, Creature? applier, CardModel? cardSource)
        {
            ...
            powerChangeStr += Math.Abs(amount);
            if (cardSource != null)
            {
